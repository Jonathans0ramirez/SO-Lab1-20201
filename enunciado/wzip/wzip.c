...
int get_max_lines(int n_files, FileTxt **files)
{
	...
	{
		file_i = files[i];
		if (file_i->n_lines > max_lines)
			max_lines = file_i->n_lines;
	}
	return max_lines;
}
...
int get_priority_index(int start, int n_lines, int *index_arr, char **lines)
{
	...
	{
		line = lines[index];
		if (line[index_arr[index]] != END_STR)
			break;
	}
	return index;
}
	...
	if (n_files > 0)
	{
		...
		for (int line = 0; line < max_lines; line++)
		{
			...
			for (int i = 0; i < n_files; i++)
			{
				file_i = files_txt[i];
				index_arr[i] = 0;
				if (line + 1 <= file_i->n_lines)
				{
					lines[i] = file_i->content[line];
				}
				else
					lines[i] = end_of_sequence;
			}
